...
GLuint setup_slices ( )
{
	GLfloat* coordarray = new GLfloat[3*3*2*num_slices];
	float space = 1.0/num_slices; 
	float s0 =-1.0;
	for(int i=0;i<num_slices*18;i+=18){
		for(int j=0;j<2*3*3;){
			coordarray[i+j] = slicearray[j++];
			coordarray[i+j] = slicearray[j++];
			coordarray[i+j] = slicearray[j++] + s0;
			coordarray[i+j] = slicearray[j++];
			coordarray[i+j] = slicearray[j++];
			coordarray[i+j] = slicearray[j++] + s0;
			coordarray[i+j] = slicearray[j++];
	...
	glBufferData(GL_ARRAY_BUFFER,num_slices*2*3*3*sizeof(GLfloat),coordarray,GL_STATIC_DRAW);
...
void draw ( )
{
  ...
  glm::mat4 PMat = glm::perspective(float(1),1.0f,18.0f,22.0f);
  ...
  glUniform3f(SizeLoc,size[0]-1,size[1]-1,size[2]-1);  // extent of the data
  ...
  glDrawArrays(GL_TRIANGLES,0,num_slices*2*3);
...
GLint init_glut(GLint *argc, char **argv)
{
  ...
  id = glutCreateWindow("Volume Render");    
...
GLint main(GLint argc, char **argv)
	...
	CubeVAO = setup_slices();

	// set up the texture here using the TextureSetup function
	TextureSetup("bonsai.raw",256,256,256);
	//TextureSetup("aneurism.raw",256,256,256);
	//TextureSetup("marschnerlobb.raw",41,41,41);
	//TextureSetup("skull.raw",256,256,256);
	//TextureSetup("lobster.raw",301,324,56);
